...
- **Er jobs stadig aktive?** I `cron/jobs.json` (OpenClaw-roden) har hvert job `enabled: true` og `state.lastStatus`. Hvis `lastStatus` er `ok` og `enabled` er true, er jobbet ikke "slået fra" — det venter bare på at gatewayen kører.

---

*Så Telegram-botten kan svare præcist når brugeren spørger hvorfor cron er stoppet.*
